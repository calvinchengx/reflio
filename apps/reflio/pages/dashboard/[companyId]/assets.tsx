...
export default function AssetsPage() {
  ...
  let fileTypes = ['SVG', 'EPS', 'PNG', 'JPG', 'JPEG', 'DOC', 'DOCX', 'CSV', 'XLS', 'XLSX', 'PDF'];
  ...
  const handleFileUpload = async (e: any, uploadTypes: string) => {
    if(planDetails === "free"){
      toast.error(() => (
        // @ts-expect-error
        <span className="text-xl">Please <a class="font-bold underline" href="/pricing">upgrade your plan</a> to upload assets for your affiliates.</span>
      ));
      return false;
    }
        ...
        <div className="pt-10 wrapper sm:flex sm:items-center sm:justify-between">
          ...
          <div>
            <input
              ...
              type="file"
              accept="image/*, application/pdf, .svg, .eps, .doc, .docx, .csv, .xls, .xlsx"
              style={{display: 'none'}}
              multiple={true}
              ref={fileInput}
            />
